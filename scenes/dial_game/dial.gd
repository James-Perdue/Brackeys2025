...
func _process(delta: float) -> void:
    ...
    elif values_ready and (value < low_warn or value > high_warn):
        warn_sent = true
        dial_warn.emit(true)
    elif warn_sent and (value > low_warn and value < high_warn):
        dial_warn.emit(false)
